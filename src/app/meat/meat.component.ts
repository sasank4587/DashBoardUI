...
export class MeatComponent implements OnInit {
  ...
  submitUpdates() {
    const updateRequests: InventoryRequest[] = this.tempInventoryCountList.map(item => {
      return new InventoryRequest('MEAT', item.entry, item.count);
    });
  ...
  saveCategory() {
    if (this.newCategory.name && this.newCategory.count >= 0) {
      const request = new InventoryRequest('MEAT', this.newCategory.name, this.newCategory.count);
  ...
  getInventoryCount() {
    this.productInventory.getInventoryCount('MEAT').subscribe(response => {
